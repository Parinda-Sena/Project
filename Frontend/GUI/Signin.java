    ...
    public Signin() {
        ...
        b.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                else if (!passwordText.equals(confirmedText)) {
                    JOptionPane.showMessageDialog(Signin, "Password doesn't match", "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                // ตรวจสอบภายในรหัสมีช่องว่าง
                else if (passwordText.equals("")) {
                    JOptionPane.showMessageDialog(Signin, "Password shouldn't blank", "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                else if (!phoneText.matches("\\d{9,10}")) {
                    JOptionPane.showMessageDialog(Signin, "Enter correct Phone Number (9 or 10 digits)");
                    return;
                }

                else if (!emailText.matches("^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+$")) {
                    JOptionPane.showMessageDialog(Signin, "Invalid email format. Please try again.");
                    return;
                }
